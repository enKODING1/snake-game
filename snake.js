class Game {
    constructor(stageWidth, backgroundColor, blockColors, fps) {
        ...
        this.stageHeight = (window.innerHeight) / (window.innerWidth / this.stageWidth);
        this.size = this.canvas.clientWidth / this.stageWidth;
        this.blockColors = blockColors;
        this.snakeHead = new Block(this.randomPos(this.stageWidth), this.randomPos(this.stageHeight), this.size, this.blockColors.head);
        this.food = new Block(this.randomPos(this.stageWidth), this.randomPos(this.stageHeight), this.size, this.blockColors.food)
        this.snakeTails = [];
        this.velocityX = 0;
        this.velocityY = 0;
        this.tail_length = 0;
    ...
    keyDownVim(e) {
        if (e.keyCode === 72 || e.keyCode === 75 || e.keyCode === 76 || e.keyCode === 74) {
            this.isStart = true;
        }
    ...
    resize() {
        ...
        this.stageHeight = (window.innerHeight) / (this.canvas.width / this.stageWidth);
    ...
    tailTouchChecker() {
        for (let i = 0; i < this.snakeTails.length; i++) {
            if (this.snakeHead.x === this.snakeTails[i].x && this.snakeHead.y === this.snakeTails[i].y) {
                alert(`score : ${this.score}`);
                this.restart()
            }
        }
    ...
    createTail() {
        ...
        if (this.snakeTails.length > this.tail_length) {
            this.snakeTails.shift();
        }
        if (this.isStart) {
            this.snakeTails.push(new Block(this.snakeHead.x, this.snakeHead.y, this.size, this.blockColors.tail));
        }
        for (let i = 0; i < this.snakeTails.length; i++) {
